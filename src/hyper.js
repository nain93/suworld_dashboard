...
$(document).ready(function() {	
	...
	socket = io(hy_ws_url, {transports:['websocket']},  {'force new connection': true, 'max reconnection attempts': 1000});
	...
	socket.on("block", function(data) {
		...
		sub = "<div class='lists-list-box aalist' style='opacity:0.1;'>"+
		"<a href='"+hy_url+"/block/"+json.block+"'>"+
		"<div class='lists-text-list'>"+
		"<b><i class='fas fa-cube'></i></b><span>#"+json.block+"<span class='block-time' data='"+json.timestamp+"'>("+json.passtime+")</span></span><span class='lists-re'>Block Reward:"+json.reward+" SUC</span>"+
		"</div><div class='lists-text-history'>"+
		"<b><i class='fas fa-exchange-alt'></i></b><span>"+json.trx_count+" Transactions</span><span class='lists-re'>Tax Reward:"+json.trx_fee+" SUC</span>"+
		"</div></a></div>";
	...
	socket.on("trx", function(data) {
		...
		sub = "<div class='lists-list-box-next'>"+
		"<a href='"+hy_url+"/tx/"+json.hash+"'>"+
		"<div class='lists-text-tran'><i class='fas fa-exchange-alt'></i><span class='text-over col-sm-8 col-md-8 col-xs-8' style='padding:0'>"+json.hash+"</span>"+
		"<span class='text-day col-sm-3 col-md-3 col-xs-3' style='padding:0;' data='"+json.timestamp+"'>"+json.passtime+"</span>"+
